...
{
    const double frequency = 800e6;  //center frequency to 500 MHz
    ...
    ifstream File4Len ("signalIanput.txt");
    if (File4Len.is_open())
    {
        while ( getline (File4Len,line) )
        {
            lenFile++;
        }
        File4Len.close();
    ...
    printf("len file = %d \n",lenFile);
    std::vector<float> input(lenFile);

    int index = 0;
    ifstream inputfile ("signalInput.txt");
    if (inputfile.is_open())
    {
        while ( getline (inputfile,line) )
        {
            input[index++] = atof(line.c_str());
            cout <<index-1<<" "<<input[index-1]<<line.c_str()<<"\n";
    ...
    auto t2 = t1;
    while (chrono::high_resolution_clock::now() - t1 < chrono::seconds(180)) //run for 10 seconds
    {
        //Transmit samples
